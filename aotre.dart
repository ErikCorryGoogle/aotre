...
class Utf8Dot extends Macro {
  ...
  void genBacktrack(State s) {
    ...
    _(s, "  %prev = getelementptr i8* %s, i32 -1");
    _(s, "  %c = load i8* %prev, align 1");
    ...
    _(s, "  %prev2 = getelementptr i8* %pos, i32 -1");
    _(s, "  %c2 = load i8* %prev2, align 1");
  ...
  void genBacktrackBackwards(State s) {
    ...
    _(s, "define internal i8* @${name}_backtrack(%restate_t* %state, i8* %s) {");
    _(s, "  %c = load i8* %s, align 1");
    _(s, "  %top_bit = and i8 %c, 128");
    _(s, "  %comparison = icmp eq i8 0, %top_bit");
    _(s, "  %next = getelementptr i8* %s, i32 1");
    _(s, "  br i1 %comparison, label %ascii, label %non_ascii");
    _(s, "ascii:");
    _(s, "  ret i8* %next");
    _(s, "non_ascii:");
    ...
    _(s, "  %next2 = getelementptr i8* %pos, i32 1");
    _(s, "  %c2 = load i8* %next2, align 1");
...
class ThreeOrFourByteUtf8 extends Ast {
  ...
  void gen(State s, String succ) {
    ...
    if (succ == null) {
      ...
      _(s, "  %next = getelementptr i8* %s, i32 %result");
...
class MultiByteBackwardsUtf8 extends Ast {
  ...
  void gen(State s, String succ) {
    _(s, "define internal i32 @$name(%restate_t* %state, i8* %s) {");
    _(s, "  %start_gep = getelementptr %restate_t* %state, i64 0, i32 0");
    _(s, "  %start = load i8** %start_gep");
...
class Backreference extends Ast {
  ...
  void gen(State s, String successor) {
    ...
    _(s, "  %start_gep = getelementptr %restate_t* %state, i64 0, i32 0");
    _(s, "  %start = load i8** %start_gep");
    _(s, "  %gep_open = getelementptr %restate_t* %state, i64 0, i32 1, i32 ${ref * 2}");
    _(s, "  %gep_close = getelementptr %restate_t* %state, i64 0, i32 1, i32 ${ref * 2 + 1}");
    _(s, "  %open = load i8** %gep_open");
    _(s, "  %close = load i8** %gep_close");
    ...
    _(s, "  ret i32 0");
    _(s, "success:");
    _(s, "  %offset = load i32* %offset_return");
    _(s, "  %next = getelementptr i8* %s, i32 %offset");
...
class CharacterClass extends Ast {
  ...
  void gen(State s, String succ) {
    ...
    if (backwards) {
      ...
      _(s, "load_char:");
      _(s, "  %gep = getelementptr i8* %s, i64 -1");
      _(s, "  %loaded_c = load i8* %gep, align 1");
    ...
    if (succ == null) {
      ...
      _(s, "  %next = getelementptr i8* %s, i64 ${backwards ? -1 : 1}");
      _(s, "  %succ_result = call i32 @$succ(%restate_t* %state, i8* %next)");
      _(s, "  ret i32 %succ_result");
    }
    _(s, "}");
...
abstract class Single extends Ast {
  ...
  void gen(State s, String succ) {
    ...
    if (offset == -1) {
      ...
      _(s, "  ret i32 ${failure_code(succ)}");
      _(s, "load_char:");
      _(s, "  %gep = getelementptr i8* %s, i64 $offset");
      _(s, "  %c = load i8* %gep, align 1");
    } else {
    ...
    _(s, "  %next = getelementptr i8* %s, i64 $advance");
    _(s, "  %succ_result = call i32 @$succ(%restate_t* %state, i8* %next)");
    _(s, "  ret i32 %succ_result");
    _(s, "}");
  }
...
class Alternative extends BinaryAst {
  ...
  void gen(State s, String succ) {
    if (succ != null) {
      ...
      _(s, "  %next = getelementptr i8* %s, i32 %left");
...
class Disjunction extends BinaryAst {
  ...
  void genBacktrack(State s) {
    _(s, "define internal i8* @${name}_backtrack(%restate_t* %state, i8* %s) {");
    _(s, "  %prev = getelementptr i8* %s, i64 ${(backwards ? 1 : -1) * l.minWidth}");
    _(s, "  ret i8* %prev");
    _(s, "}");
    l.genBacktrack(s);
    r.genBacktrack(s);
  ...
  void ClearCaptures(State s) {
    ...
    for (int reg in captures) {
      if (s.isCaptureRepresentation(reg)) {
        ...
        for (int i = r; i < r + 2; i++) {
          _(s, "  %gep$i = getelementptr %restate_t* %state, i64 0, i32 1, i32 $i");
...
class Capturing extends UnaryAst {
  ...
  void gen(State s, String succ) {
    ...
    _(s, "define internal i32 @$name(%restate_t* %state, i8* %s) {");
    _(s, "  %gep = getelementptr %restate_t* %state, i64 0, i32 1, i32 $first");
    ...
    _(s, "  %gep = getelementptr %restate_t* %state, i64 0, i32 1, i32 $second");
    _(s, "  store i8* %s, i8** %gep");
...
class Loop extends UnaryAst {
    ...
    for (int reg in regs) {
      if (s.isCaptureRepresentation(reg)) {
        ...
        for (int i = capture; i < capture + 2; i++) {
          _(s, "  %gep_capture$i = getelementptr %restate_t* %state, i64 0, i32 1, i32 $i");
          _(s, "  %cap$i = load i8** %gep_capture$i");
          _(s, "  store i8* null, i8** %gep_capture$i");
        }
      } else if (s.isCounterRepresentation(reg)) {
        int count = s.getCounter(reg);
        _(s, "  %gep_count$count = getelementptr %restate_t* %state, i64 0, i32 2, i32 $count");
        _(s, "  %count$count = load i32* %gep_count$count");
        _(s, "  store i32 0, i32* %gep_count$count");
      } else {
        int z = s.getZeroCheck(reg);
        _(s, "  %gep_zero_check$z = getelementptr %restate_t* %state, i64 0, i32 3, i32 $z");
        _(s, "  %zero_check$z = load i8** %gep_zero_check$z");
  ...
  void genPreCounter(State s) {
    if (counted) {
      _(s, "  %gep = getelementptr %restate_t* %state, i64 0, i32 2, i32 ${counter_register}");
      _(s, "  %counter = load i32* %gep");
    }
    if (zeroChecked) {
      int z = zero_check_register;
      _(s, "  %gep_zero_check = getelementptr %restate_t* %state, i64 0, i32 3, i32 $z");
      _(s, "  %zero_check = load i8** %gep_zero_check");
      if (min != 0) {
...
class Parser {
  ...
  void checkForNull() {
    if (currentCodePoint == 0) die("Can't allow null characters in an Aotre regexp");
  }
  ...
  Ast parse() {
    ...
    try {
      ...
      stderr.writeln("Aotre error: ${error.string} at ${error.pos}");
      stderr.writeln(src);
  ...
  int acceptAsciiEscape() {
    ...
    if (accept("x") || accept("u")) {
      ...
      if (hex == 0) die("Can't allow null characters in an Aotre regexp");
  ...
  Ast escapeGate(int code) {
    if (code == 0) die("Can't allow null characters in an Aotre regexp");
...
void defineTopLevel(State state, String symbol, String name) {
  ...
  out.writeln("  %start_gep = getelementptr %restate_t* %state, i64 0, i32 0");
  out.writeln("  store i8* %s, i8** %start_gep");
  for (int i = 0; i < state.captures; i++) {
    out.writeln("  %capture_gep$i = getelementptr %restate_t* %state, i64 0, i32 1, i32 $i");
    out.writeln("  store i8* null, i8** %capture_gep$i");
  }
  for (int i = 0; i < state.counters; i++) {
    out.writeln("  %counter_gep$i = getelementptr %restate_t* %state, i64 0, i32 2, i32 $i");
    out.writeln("  store i32 0, i32* %counter_gep$i");
  }
  for (int i = 0; i < state.zeroChecks; i++) {
    out.writeln("  %zero_check_gep$i = getelementptr %restate_t* %state, i64 0, i32 3, i32 $i");
...
void usage() {
  ...
  stderr.writeln("  [-s <symbol>]       (default 'aotre')");
  stderr.writeln("  [-m <mode>]         (default 'aotre', can also be 'js' or 'perl'");
}

void main(List<String> args) {
  String source;
  String topSymbol = "aotre";
  String filename;
  bool dotFile = false;
  bool llFile = false;
  String mode = "aotre";
